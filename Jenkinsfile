node {
   ...
   def mvnHome = tool 'M3'
    ...
    dir('App') {
        ...
        def v = version()
        sh "${mvnHome}/bin/mvn clean package -DreleaseVersion=${v} -DdevelopmentVersion=${pom.version} -DskipTests -Pdocker docker:push -t ashishkarpe\pomeloassignment1:Pomelo-App"
    }
}
def version() {
